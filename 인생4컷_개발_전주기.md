...
> 🔒 **정보보호 안내**: 본 문서에 기재된 모든 민감 정보(Google Service Account Key, 개인 계정 이메일, 클라우드 폴더 고유 ID, 로컬 PC 절대 경로, 웹훅 토큰 등)는 외부 유출 방지 및 보안 준수를 위해 **마스킹(Masking)** 처리되었습니다.
...
│   ├── google-key.json                # (비공개) 구글 서비스 계정 OAuth 인증 키 [보안 격리]
...
* **핵심 함수 및 컴포넌트**:
  * `MODEL_REGISTRY`:
    ...
    * `SG161222/Realistic_Vision_V5.1_noVAE`: 마법사 아카데미(`wizard`), 네온 판타지(`neon_fantasy`), 흑백 시네마(`bw_cinema`), 오리지널(`original`) 실사 특화.
  ...
  * `STYLE_CONFIGS`:
    * 6개 테마별 최적의 `strength` 계수(`0.28 ~ 0.32`), `guidance_scale: 7.0 ~ 7.5`, Positive/Negative Prompt 테이블 정의.
...
sequenceDiagram
    ...
    LocalServer->>PhotoDrive: 사진 업로드 (ID: 13KXZ_***[MASKED])
    LocalServer->>VideoDrive: 영상 업로드 (ID: 1RgvKVq-***[MASKED])
...
* **주요 개선 내용**:
  1. **구글 드라이브 스토리지 분리**: 사진 폴더(`13KXZ_***`)와 영상 폴더(`1RgvKVq-***`)로 분리 업로드.
...
### 4.6 [6차 개발] 신규 <마법사 테마> 추가 & 다운로드 즉시/24h 파기 2중 개인정보 보호 알고리즘
...
* **개발 목표**: 판타지 테마 라인업 확장(마법사 테마), CLIP 토큰 한도 최적화, 다운로드 후 즉시 파기 2중 보안 체계 수립.
* **주요 개선 내용**:
  1. **신규 <마법사 테마 (Wizard)> 추가 및 6대 카테고리 확장**:
     * Realistic Vision v5.1 기반 8K 극실사 룩, 다크 위저드 로브, 은은하게 빛나는 마법 지팡이, 고대 고딕 마법 도서관(부유 마법서, 깃펜, 빛나는 파티클, 촛불 앰비언트) 배경 전이.
     * `strength: 0.35` 정밀 튜닝으로 인물 이목구비/표정은 100% 보존하면서 배경을 고딕 도서관으로 확실하게 전환.
  2. **CLIP 77 토큰 한도 초과 경고 완전 해소**:
     * 네거티브 및 포지티브 프롬프트의 토큰 수를 20~38개로 정밀 압축하여 77토큰 제한 내에 완벽하게 수용.
  3. **다운로드 즉시 파기(60초 유예) & 24시간 자동 파기 2중 보안 알고리즘 탑재**:
     * 모바일에서 사진 또는 비하인드 동영상 다운로드 시 백엔드 `POST /api/cleanup-session`이 호출되어 60초 유예 후 해당 세션의 로컬 파일(`ai_frame_*`, `orig_frame_*`, `orig_single_*`, `behind_video_*`, `temp_*`)을 영구 파기.
     * 미다운로드 시에도 `APScheduler` 기반으로 24시간 경과 파일 및 30분 초과 임시 파일을 100% 자동 영구 삭제.
...
4. **키(JSON) 발급 및 보안 격리:**
   ...
   - ⚠️ **보안 주의**: 다운로드된 JSON 키 파일(`private_key` 포함)은 GitHub 등 공개 저장소에 절대 Commit되지 않도록 `.gitignore`에 등록 관리합니다.
...
2. **구글 드라이브 폴더 생성 및 서비스 계정 공유 (필수):**
   ...
   - 각 폴더의 `[공유]` 설정 클릭 ➔ `google-key.json` 내부에 적힌 `client_email` 주소(예: `4cuts-storage-bot@[PROJECT_ID].iam.gserviceaccount.com`)를 붙여넣고 **편집자(Editor)** 권한으로 공유.
   - 각 폴더 URL 끝부분의 고유 폴더 ID를 확인하여 `local-server/app.py`에 등록:
     - `GOOGLE_PHOTO_FOLDER_ID = "[PHOTO_FOLDER_ID_MASKED]"`
     - `GOOGLE_VIDEO_FOLDER_ID = "[VIDEO_FOLDER_ID_MASKED]"`
...
1. **GAS 프로젝트 생성:**
   - [Google Apps Script](https://script.google.com/)에 본인 구글 계정(`[USER_ACCOUNT]@gmail.com`)으로 로그인 후 `[새 프로젝트]` 생성.
...
2. **배포 매개변수 설정:**
   - 유형: **`웹 앱`**
   - 설명: `인생4컷 저장소 v2.1`
   - 다음 사용자 권한으로 실행: **`나 ([USER_ACCOUNT]@gmail.com)`**
   - 액세스 권한이 있는 사용자: **`모든 사용자 (Anyone)`** *(반드시 Anyone으로 지정해야 403 Forbidden 에러 방지)*
3. **Webhook URL 코드 반영:**
   - 배포 완료 후 발급된 **웹 앱 URL (`https://script.google.com/macros/s/[DEPLOYMENT_ID_MASKED]/exec`)**을 복사.
