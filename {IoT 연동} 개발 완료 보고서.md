...
### 4.3. 현장 세팅 모니터링 및 진단 도구 사용법 (신규 추가)
1. **홈 화면 퀵 상태 캡슐**:
   - 태블릿 화면 상단에 `🟢 네트워크 정상 (00ms) ⚙️` 캡슐 뱃지가 15초 주기로 자동 갱신됩니다.
2. **현장 네트워크 & IoT 연동 진단 모달 열기**:
   - 상단 캡슐 뱃지를 터치하면 진단 대시보드가 열립니다.
   - **태블릿 ⇄ Host PC 통신**: 양 장비의 IP와 동일 Wi-Fi 서브넷 일치 여부, HTTP API 왕복 핑(Latency) 확인.
   - **CP1500 인화기**: Windows 스풀러 드라이버 감지 상태 및 18매 카세트 잔여 용지 매수 확인.
   - **클라우드 & AI**: Google Drive 및 OpenAI API 통신 상태 점검.
3. **[🧪 테스트 엽서 1장 출력]**:
   - 본 행사 개시 전 버튼 1번 터치로 4색(Yellow, Magenta, Cyan, Black) 컬러 밴드와 300 DPI 정렬 격자가 담긴 엽서 시험 인쇄를 즉시 진행하여 하드웨어 발색 및 용지 세팅을 사전 점검합니다.
...
- 태블릿과 Host PC, CP1500 간의 무선 통신 상태를 실시간 진단하고 시험 인쇄할 수 있는 **현장 진단 모니터링 대시보드**를 추가 탑재하여 운영 안정성을 대폭 강화했습니다.
- 모든 소스코드가 로컬 Git에 안전하게 커밋되었습니다.
